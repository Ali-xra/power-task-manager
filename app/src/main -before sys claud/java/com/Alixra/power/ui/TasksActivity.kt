...
class TasksActivity : AppCompatActivity() {
    ...
    private fun setupRecyclerView() {
        tasksAdapter = TasksAdapter { task, isCompleted ->
    ...
    private fun setupClickListeners() {
        ...
        thisWeekCard.setOnClickListener {
            currentPeriod = TimePeriod.THIS_WEEK
            showTasksView()
        }

        thisMonthCard.setOnClickListener {
            currentPeriod = TimePeriod.THIS_MONTH
            showTasksView()
        }

        thisSeasonCard.setOnClickListener {
            currentPeriod = TimePeriod.THIS_SEASON
            showTasksView()
        }
    ...
    private fun getTasksForTimePeriod(timePeriod: TimePeriod): List<Task> {
        return prefsManager.getAllTasks().filter { task ->
            task.timePeriod == timePeriod && timePeriod.isDateInPeriod(task.createdAt)
        }
    }
    ...
    private fun showAddTaskDialog() {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("افزودن") { _, _ ->
                ...
                if (title.isNotEmpty()) {
                    // استفاده از یک بخش پیش‌فرض اگر هیچ بخشی وجود نداشت
